...
export const handler = async (
  ...
  logger.info(`udacity: event.header ${event.headers}, event.token ${event.authorizationToken}`, event)
  try {
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  // learned from: https://knowledge.udacity.com/questions/203918
  // You should implement it similarly to how it was implemented for the exercise for the lesson 5
  // You can read more about how to do this here: https://auth0.com/blog/navigating-rs256-and-jwks/
  let key = await getSigningKey(jwksUrl, jwt.header.kid)
  logger.info(`got signing key: ${key}`)
...
const getSigningKey = async (jwkurl, kid) => {
  ...
  logger.info(`Axios jwksurl res: ${res}`)
